...
export default function CreateTaskDialog({ showCreateTask, setShowCreateTask, projectId }) {
    ...
    const handleSubmit = async (e) => {
        ...
        function handleClickOutside(event) {
            if (createForm.current && !createForm.current.contains(event.target)) {
    ...
    return showCreateTask ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 dark:bg-black/60 backdrop-blur">
            ...
            <div className="relative bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-lg shadow-lg w-full max-w-md p-6 text-zinc-900 dark:text-white"
             ref={createForm}
            >
                 <button className="absolute top-3 right-3 text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200" onClick={()=>setShowCreateTask(false)}><XIcon/></button>
